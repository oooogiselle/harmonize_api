...
router.get('/api/recommendations', async (req, res) => {
  try {
    ...
    if (seedArtists.length === 0 && seedTracks.length === 0 && seedGenres.length === 0)
      return res.status(204).json([]); // no listening data yet
    ...
    try {
      ...
      console.warn('[Spotify] recs (genres) 404:', JSON.stringify(e.body || e));
    }

    /* ---------- 5. Final fallback: any available genre ---------- */
    try {
      const genreSeedRes  = await spotify.getAvailableGenreSeeds();
      const fallbackGenres = genreSeedRes.body.genres.slice(0, 5);      // guaranteed valid
