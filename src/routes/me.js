...
router.get('/api/me/spotify', async (req, res) => {
  ...
  async function getSpotifyData(api) {
    const [profile, topTracks, topArtists, recent] = await Promise.all([
      api.getMe(),
      api.getMyTopTracks({ limit: 10 }),
      api.getMyTopArtists({ limit: 10 }),
      api.getMyRecentlyPlayedTracks({ limit: 10 }),
    ]);
    return {
  ...
  try {
    const data = await getSpotifyData(spotify);
    res.json(data);
  } catch (err) {
    if (err.statusCode === 401) {
      try {
        const refreshedSpotify = await refreshAccessTokenForUser(user);
        const data = await getSpotifyData(refreshedSpotify);
        res.json(data);
      } catch (refreshErr) {
...
router.get('/api/recommendations', async (req, res) => {
  ...
  async function getRecommendations(api) {
    const top = await api.getMyTopArtists({ limit: 5, time_range: 'medium_term' });
    const rec = await api.getRecommendations({
      seed_artists: top.body.items.map((a) => a.id),
      limit: 20,
    });
    return rec.body.tracks.map(mapTrack);
  }

  try {
    const data = await getRecommendations(spotify);
    res.json(data);
  } catch (err) {
    if (err.statusCode === 401) {
      try {
        const refreshedSpotify = await refreshAccessTokenForUser(user);
        const data = await getRecommendations(refreshedSpotify);
        res.json(data);
      } catch (refreshErr) {
        console.error('Refresh token failed', refreshErr);
        return res.status(500).json({ error: 'Spotify refresh failed' });
...
router.get('/api/recent', async (req, res) => {
  ...
  async function getRecent(api) {
    const recent = await api.getMyRecentlyPlayedTracks({ limit: 20 });
    return recent.body.items.map((i) => mapTrack(i.track));
  }

  try {
    const data = await getRecent(spotify);
    res.json(data);
  } catch (err) {
    if (err.statusCode === 401) {
      try {
        const refreshedSpotify = await refreshAccessTokenForUser(user);
        const data = await getRecent(refreshedSpotify);
        res.json(data);
      } catch (refreshErr) {
        console.error('Refresh token failed', refreshErr);
        return res.status(500).json({ error: 'Spotify refresh failed' });
